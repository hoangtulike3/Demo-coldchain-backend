...
class RoomRepo {
    // 채팅방 전체 조회
    async getRooms(userId, page, pageSize, searchName) {
    ...
    `;
        const rooms = await db_1.db.manyOrNone(query, [userId, pageSize, offset, searchName]);
    ...
    async addRoom(room) {
        ...
        console.log(room);
        let createdRoom;
        await db_1.db.tx(async (t) => {
            const roomInsertQuery = `SET TIME ZONE 'Asia/Seoul'; ` +
                `
        ...
        `;
            ...
            for (const user_id of participants) {
                await t.none(`SET TIME ZONE 'Asia/Seoul'; ` +
                    `
    ...
    // 채팅방 수정
    async modifyRoom(room, roomId, taskId) {
        const currentRoom = await this.getRoom(roomId);
        const updatedTasks = currentRoom.tasks || [];
        updatedTasks.push(taskId);
        await db_1.db.any(`UPDATE room SET tasks = $1, updated_at = current_timestamp WHERE id = $2 RETURNING *`, [
            updatedTasks,
            roomId,
        ]);
    ...
    // 채팅방 참여자 확인
    static async isParticipant(userId, roomId) {
    ...
    // 전체 채팅방 중 본인 확인
    static async isParticipantById(userId) {
        const result = await db_1.db.oneOrNone(`
      SELECT 
        1 
      FROM "user_room"
