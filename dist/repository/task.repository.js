...
class TaskRepo {
    // 테스크 전체 조회
    async getTasks(userId, page, pageSize, searchName) {
    ...
    `;
        const tasks = await db_1.db.manyOrNone(query, [userId, pageSize, offset, searchName]);
    ...
    // 테스크 참여자 확인
    async isParticipant(userId, taskId) {
