...
// load .env files
if (dotenv_1.default.config({ path: `.env/.env.${ENV}` }).error)
    throw new Error(`Environment file not found (.env.${ENV})`);
if (dotenv_1.default.config({ path: `.env/.env` }).error)
    throw new Error(`Environment file not found (.env)`);
// set default value
if (!process.env.TZ)
    process.env.TZ = "UTC";
