...
class PackageRepo {
  ...
  async getPackages(
    ...
    const params: any[] = [pageSize, offset, placeId, categoryId, contentId, statusId, wareId];
  ...
  // Delete Package
  async deletePhotoOnPackageDeleted(packageId: string) {
    const result = await db.oneOrNone(
      `
      DELETE
      FROM photos
  ...
  // Delete Package
  async deletePhoto(packageId: string, values: object) {
    const result = await db.oneOrNone(`DELETE FROM photos WHERE package_id = $1 and photo_url NOT IN ($2:csv)`, [
      packageId,
      values,
    ]);
    return result;
  ...
  async deleteAllPhoto(packageId: string) {
    const result = await db.oneOrNone(`DELETE FROM photos WHERE package_id = $1`, [packageId]);
    return result;
  }
  ...
  // 패키지 수정
  async modifyPackage(packages: object, packageId: string) {
    await db.any(
      pgp.helpers.update(packages, null, "package") +
  ...
  async getPackageFilterList() {
    const packageCategories = await db.manyOrNone(`
      SELECT id, name FROM package_category
      ORDER BY created_at DESC
    `);

    const packageContents = await db.manyOrNone(`
      SELECT id, name FROM package_content
      ORDER BY created_at DESC
    `);

    const packageWares = await db.manyOrNone(`
      SELECT id, name FROM package_ware
      ORDER BY created_at DESC
    `);

    const packageStatus = await db.manyOrNone(`
  ...
  // 패키지 사진 추가
  async addPackagePhoto(photo_url: string, package_id: string) {
    const result = await db.oneOrNone(
      `SET TIME ZONE 'Asia/Seoul'; ` +
