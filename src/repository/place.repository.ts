...
export class PlaceRepo {
  ...
  // 플레이스 수정
  async modifyPlace(place: object, placeId: string) {
    await db.any(
      pgp.helpers.update(place, null, "place") + `, updated_at = current_timestamp WHERE id = '${placeId}' RETURNING *`
    );
  ...
  // 플레이스 타입 조회
  async getPlaceType() {
    const serviceTypesResult = await db.manyOrNone(`
      SELECT id, name FROM place_service_type
      ORDER BY created_at DESC
    `);

    const stationTypesResult = await db.manyOrNone(`
      SELECT id, name FROM place_station_type
      ORDER BY created_at DESC
    `);

    const warehouseTypesResult = await db.manyOrNone(`
  ...
  // 알람 토글
  async toggleNotification(placeId: string) {
    const result = await db.manyOrNone(
      `
        UPDATE "place"
  ...
  // 플레이스 사진 추가
  async addPlacePhoto(photo_url: string, place_id: string) {
    const result = await db.oneOrNone(
      `SET TIME ZONE 'Asia/Seoul'; ` +
